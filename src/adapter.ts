...
function httpErrorCode(status: number, error?: WireErrorBody['error']): string {
...
/** The bearer credential for one request, and the pool entry it came from. */
...
export class KeyBalAdapter extends LlmAdapter {
  constructor(private readonly routes: () => ReadonlyMap<string, KeyBalRoute>) {
    super()
  }
  ...
  private acquireCredential(route: KeyBalRoute, model: string): Credential | null {
  ...
  private async requestOnce(
    ...
    body: ReturnType<typeof serializeRequest>,
    options: GenerateOptions,
  ): Promise<{ ok: boolean; status: number; chunks: StreamChunk[] }> {
...
export function buildRoutes(config: ResolvedKeyBalConfig): Map<string, KeyBalRoute> {
  const routes = new Map<string, KeyBalRoute>()
